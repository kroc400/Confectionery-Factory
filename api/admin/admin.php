...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* САЙДБАР – на мобильных становится горизонтальным */
        .admin-sidebar { 
            width: 100%; 
        ...
        /* СТАТИСТИКА – сетка для мобильных */
        .admin-stats-grid { 
            display: grid; 
        ...
        .admin-report-card table { 
            width: 100%; 
            border-collapse: collapse; 
        }
        
        .admin-report-card td { 
            padding: 6px 0; 
            font-size: 0.8rem;
        }
        ...
        @media (min-width: 768px) {
            ...
            .admin-stat-title { font-size: 0.85rem; }
            ...
            .admin-data-table table { min-width: auto; }
        }
        ...
        <main class="admin-content">
            ...
            ?>
                ...
                <div class="admin-data-table">
                    <form method="POST" id="userEditForm">
                        ...
                        <table>
                            ...
                            <tbody>
                                ...
                                <tr data-user-id="<?= $u['id'] ?>">
                                    ...
                                    <td><button type="button" class="admin-edit-btn" onclick="editUser(<?= $u['id'] ?>, '<?= htmlspecialchars($u['email']) ?>', '<?= htmlspecialchars($u['name']) ?>', '<?= htmlspecialchars($u['phone']) ?>', <?= $u['is_wholesale'] ?>, '<?= $u['role'] ?>')">Ред.</button></td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
                        <div id="editFields" style="display:none;">
                            ...
                            <div style="display:grid; gap:10px; grid-template-columns:1fr; margin-top:15px;">
                                <div><label>Email</label><input type="email" name="email" id="edit_email" class="admin-edit-input"></div>
                                <div><label>Имя</label><input type="text" name="name" id="edit_name" class="admin-edit-input"></div>
                                <div><label>Телефон</label><input type="text" name="phone" id="edit_phone" class="admin-edit-input"></div>
                                <div><label>Оптовик</label><select name="is_wholesale" id="edit_wholesale" class="admin-edit-select"><option value="0">Нет</option><option value="1">Да</option></select></div>
                                <div><label>Роль</label><select name="role" id="edit_role" class="admin-edit-select"><option value="user">Пользователь</option><option value="admin">Админ</option></select></div>
                            </div>
                ...
                <script>
                    ...
                    function cancelEdit() { document.getElementById('editFields').style.display = 'none'; document.getElementById('edit_user_id').value = ''; }
                </script>
